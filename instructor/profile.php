<?php
	...
	if(isset($_SESSION['role']) && $_SESSION['role'] == 'instructor'){
      ...
      $fetch = mysqli_query($db, "SELECT password FROM tb_instructor WHERE employeeNo = '{$employeeNo}'");
      $row = mysqli_fetch_assoc($fetch);
      ...
      if($counter > 0 && $row['password'] === $currPassword){
       ...
       if (!empty($_FILES["profPic"]["name"])) {
              ...
              $imagename=$_SESSION['empno'].$ext;

             $target_path = "../uploads/".$imagename;
            
          if(file_exists("../uploads/".$imagename)) unlink("../uploads/".$imagename);
          if(move_uploaded_file($temp_name, $target_path)) {
             $query_upload="UPDATE tb_instructor set profilePic = '{$imagename}' WHERE employeeNo = '{$employeeNo}' AND password = '{$currPassword}'";
        ...
        $editInst = mysqli_query($db, "UPDATE tb_instructor SET username = '{$username}', firstname = '{$firstname}', lastname = '{$lastname}', 
    ...
    //$fetch = mysqli_query($db, "SELECT * FROM tb_instructor WHERE username = '{$_SESSION['username']}'");
    //$row = mysqli_fetch_assoc($fetch);
	}else{
    ...
    <div class="jumbotron">
      <div class="container_user col-md-10">
        ...
        <center>
          ...
          <div class="form-group input-group" id="inst-input" >
          ...
          <div class="form-group input-group" id="inst-input" >
             <span class="input-group-addon">Employee No.</span>
             <input type="text" name="employeeNo" id="employeeNo" class="form-control" value=<?php echo $row['employeeNo'];?> maxlength="10" readonly/>
          </div> 
          ...
          <div class="form-group input-group" id="inst-input" >
             ...
             <input type="text" id="accountType" class="form-control" value="Instructor" disabled="disabled" />
          </div> 
